...
func (c *Connection) NegotiateProtocol() error {
	...
	//fmt.Println(hex.EncodeToString(negRes.ProtocolID))
	if hex.EncodeToString(negRes.ProtocolID) == "fe534d42" {
		c.Session.Version = "SMB2"
		//fmt.Println("SMB Protocol Version: SMB2")
	} else if hex.EncodeToString(negRes.ProtocolID) == "ff534d42" {
		c.Session.Version = "SMB1"
		//fmt.Println("SMB Protocol Version: SMB1")
	} else {
		fmt.Println("unknown SMB protocol version")
	}
